...
def map_device_status(val: Any) -> str:
    ...
    Returns:
        Mapped device type or 'Both' as default
    """
    if pd.isna(val):
        return "Both"

    val = int(val) if isinstance(val, (int, float)) else val
    return DEVICE_MAPPINGS.get(val, "Both")
...
def map_automation_status_java(val: Any) -> Optional[str]:
    ...
    Args:
        val: Java automation status ID

    Returns:
        Mapped automation status or None
    """
    if pd.isna(val):
        return None

    val = int(val) if isinstance(val, (int, float)) else val
    return JAVA_STATUS_MAPPINGS.get(val, None)
...
def map_automation_status_testim(val: Any) -> Optional[str]:
    ...
    Args:
        val: Testim automation status ID
    ...
    return TESTIM_STATUS_MAPPINGS.get(val, None)
